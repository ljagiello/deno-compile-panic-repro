    ...
    this.uploadFileStream = async function (fileStream, meta, encryptionMetadata) {
        ...
        try {
            if (this.shouldUseJsonApi(meta)) {
                ...
                // `file.save(...)` expects a Buffer/string. For streams, use `createWriteStream(...)`.
                if (Buffer.isBuffer(fileStream) || typeof fileStream === 'string') {
                    await gcsFile.save(fileStream, jsonApiOptions);
                }
                else {
                    const writeStream = gcsFile.createWriteStream(jsonApiOptions);
